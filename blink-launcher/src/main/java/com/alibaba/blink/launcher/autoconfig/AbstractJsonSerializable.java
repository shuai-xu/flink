...
public class AbstractJsonSerializable {
	...
	public static <T> T fromJson(File file, Class c) {
		try {
			return fromJson(FileUtils.readFileUtf8(file), c);
		} catch (IOException e) {
			throw new IllegalArgumentException("error converting from JSON file", e);
		}
	}
