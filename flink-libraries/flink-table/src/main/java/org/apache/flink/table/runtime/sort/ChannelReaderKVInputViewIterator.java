...
public class ChannelReaderKVInputViewIterator<K, V> implements MutableObjectIterator<Tuple2<K, V>> {
	...
	public ChannelReaderKVInputViewIterator(
		...
		this(ioAccess, channel, new LinkedBlockingQueue(), segments, freeMemTarget, keySerializer, valueSerializer, numBlocks);
	}
	...
	public ChannelReaderKVInputViewIterator(
			...
			TypeSerializer<V> valueSerializer,
			int numBlocks) throws IOException {
		this(ioAccess.createBlockChannelReader(channel, returnQueue), returnQueue, segments, freeMemTarget, keySerializer, valueSerializer, numBlocks);
	}
