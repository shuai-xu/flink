...
import org.apache.flink.table.runtime.operator.join.batch.hashtable.BaseHybridHashTable;
import org.apache.flink.table.runtime.operator.join.batch.hashtable.ProbeIterator;
...
import static org.apache.flink.table.runtime.operator.join.batch.hashtable.longtable.LongHashPartition.INVALID_ADDRESS;
...
public abstract class LongHybridHashTable extends BaseHybridHashTable implements LongHashContext {
	...
	private void tryDenseMode() {
		...
		if (writeBehindBuffersAvailable != 0) {
			throw new RuntimeException("writeBehindBuffersAvailable should be 0: " + writeBehindBuffersAvailable);
		}
		...
		if (range <= recordCount * 4 || range <= segmentSize / 8) {
			...
			this.minKey = minKey;
			this.maxKey = maxKey;
			writeBehindBuffers.drainTo(availableMemory);
	...
	private boolean prepareNextPartition() throws IOException {
		...
		for (final LongHashPartition p : this.partitionsBeingBuilt) {
			buffersAvailable += p.finalizeProbePhase(this.partitionsPending);
		}
		...
		if (totalBuffersNeeded < totalBuffersAvailable) {
			...
			while (true) {
				try {
					...
					this.availableMemory.addAll(inView.close());
					break;
				}
			}
