...
/**
  * Base class for merge predicates to IN or NOT_IN.
  */
abstract class MergeToNotInOrInRule(description: String) extends RelOptRule(
  ...
  def needRewrite(head: RexNode, size: Int): Boolean = {
    FlinkTypeFactory.toInternalType(head.getType) match {
      case DataTypes.FLOAT | DataTypes.DOUBLE => size >= FRACTIONAL_THRESHOLD
  ...
  protected def convertToNotInOrIn(
    ...
    combineMap.values.foreach { list =>
      ...
      if (needRewrite(inputRef, list.size)) {
