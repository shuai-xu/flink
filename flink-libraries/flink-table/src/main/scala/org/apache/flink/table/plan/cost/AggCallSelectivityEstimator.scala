...
import org.apache.flink.table.plan.cost.SelectivityEstimator._
...
class AggCallSelectivityEstimator(agg: RelNode, mq: FlinkRelMetadataQuery)
  ...
  private def estimateEquals(inputRef: RexInputRef, literal: RexLiteral): Option[Double] = {
    ...
    if (!canConvertToNumericType(inputRef.getType) || aggCall.isEmpty) {
    ...
    val convertedInterval = convertValueInterval(aggCallInterval, inputRef.getType)
    convertedInterval match {
      ...
      case i: FiniteValueInterval =>
        val min = comparableToDouble(i.lower)
        val max = comparableToDouble(i.upper)
        if (ValueInterval.contains(i, literalToComparable(literal))) {
  ...
  private def estimateComparison(
    ...
    val aggCall = getSupportedAggCall(inputRef.getIndex)
    if (canConvertToNumericType(inputRef.getType) && aggCall.isDefined) {
