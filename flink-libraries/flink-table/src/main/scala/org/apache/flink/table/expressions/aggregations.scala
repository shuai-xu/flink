...
case class Sum(child: Expression) extends Aggregation {
  ...
  override private[flink] def toAggCall(name: String)(implicit relBuilder: RelBuilder): AggCall =
    relBuilder.aggregateCall(
      SqlStdOperatorTable.SUM, false, false, null, name, child.toRexNode)
...
case class Sum0(child: Expression) extends Aggregation {
  ...
  override private[flink] def toAggCall(name: String)(implicit relBuilder: RelBuilder): AggCall =
    relBuilder.aggregateCall(
      SqlStdOperatorTable.SUM0, false, false, null, name, child.toRexNode)
...
case class Min(child: Expression) extends Aggregation {
  ...
  override private[flink] def toAggCall(name: String)(implicit relBuilder: RelBuilder): AggCall =
    relBuilder.aggregateCall(
      SqlStdOperatorTable.MIN, false, false, null, name, child.toRexNode)
...
case class Max(child: Expression) extends Aggregation {
  ...
  override private[flink] def toAggCall(name: String)(implicit relBuilder: RelBuilder): AggCall =
    relBuilder.aggregateCall(
      SqlStdOperatorTable.MAX, false, false, null, name, child.toRexNode)
...
case class Count(child: Expression) extends Aggregation {
  ...
  override private[flink] def toAggCall(name: String)(implicit relBuilder: RelBuilder): AggCall =
    if (isWildcard) {
      relBuilder.aggregateCall(
        SqlStdOperatorTable.COUNT, false, false, null, name)
    } else {
      relBuilder.aggregateCall(
        SqlStdOperatorTable.COUNT, false, false, null, name, child.toRexNode)
    }
...
case class Avg(child: Expression) extends Aggregation {
  ...
  override private[flink] def toAggCall(name: String)(implicit relBuilder: RelBuilder): AggCall =
    relBuilder.aggregateCall(
      SqlStdOperatorTable.AVG, false, false, null, name, child.toRexNode)
...
case class Collect(child: Expression) extends Aggregation  {
  ...
  override private[flink] def toAggCall(name: String)(implicit relBuilder: RelBuilder): AggCall =
    relBuilder.aggregateCall(
      SqlStdOperatorTable.COLLECT, false, false, null, name, child.toRexNode)
...
case class Rank() extends Aggregation {
  ...
  override private[flink] def toAggCall(name: String)(implicit relBuilder: RelBuilder): AggCall =
    relBuilder.aggregateCall(
      SqlStdOperatorTable.RANK, false, false, null, name)
...
case class DenseRank() extends Aggregation {
  ...
  override private[flink] def toAggCall(name: String)(implicit relBuilder: RelBuilder): AggCall =
    relBuilder.aggregateCall(
      SqlStdOperatorTable.DENSE_RANK, false, false, null, name)
...
case class RowNumber() extends Aggregation {
  ...
  override private[flink] def toAggCall(name: String)(implicit relBuilder: RelBuilder): AggCall =
    relBuilder.aggregateCall(
      SqlStdOperatorTable.ROW_NUMBER, false, false, null, name)
...
case class Lead(exp: Expression, offset: Literal = Literal(1),
  ...
  override private[flink] def toAggCall(name: String)(implicit relBuilder: RelBuilder): AggCall =
    relBuilder.aggregateCall(
      SqlStdOperatorTable.LEAD, false, false, null,
      name, children.map(_.toRexNode): _*)
...
case class Lag(exp: Expression, offset: Literal = Literal(1),
  ...
  override private[flink] def toAggCall(name: String)(implicit relBuilder: RelBuilder): AggCall =
    relBuilder.aggregateCall(
      SqlStdOperatorTable.LAG, false, false, null,
      name, children.map(_.toRexNode): _*)
...
case class Stddev(child: Expression) extends Aggregation {
  ...
  override private[flink] def toAggCall(name: String)(implicit relBuilder: RelBuilder): AggCall =
    relBuilder.aggregateCall(
      SqlStdOperatorTable.STDDEV, false, false, null, name, child.toRexNode)
...
case class VarPop(child: Expression) extends Aggregation {
  ...
  override private[flink] def toAggCall(name: String)(implicit relBuilder: RelBuilder): AggCall =
    relBuilder.aggregateCall(
      SqlStdOperatorTable.VAR_POP, false, false, null, name, child.toRexNode)
...
case class Variance(child: Expression) extends Aggregation {
  ...
  override private[flink] def toAggCall(name: String)(implicit relBuilder: RelBuilder): AggCall =
    relBuilder.aggregateCall(
      SqlStdOperatorTable.VARIANCE, false, false, null, name, child.toRexNode)
...
case class FirstValue(child: Expression) extends Aggregation {
  ...
  override private[flink] def toAggCall(name: String)(implicit relBuilder: RelBuilder): AggCall =
    relBuilder.aggregateCall(
      AggSqlFunctions.FIRST_VALUE, false, false, null, name, child.toRexNode)
...
case class LastValue(child: Expression) extends Aggregation {
  ...
  override private[flink] def toAggCall(name: String)(implicit relBuilder: RelBuilder): AggCall =
    relBuilder.aggregateCall(
      AggSqlFunctions.LAST_VALUE, false, false, null, name, child.toRexNode)
...
case class SingleValue(child: Expression) extends Aggregation {
  ...
  override private[flink] def toAggCall(name: String)(implicit relBuilder: RelBuilder): AggCall =
    relBuilder.aggregateCall(
      getSqlAggFunction, false, false, null, name, child.toRexNode)
...
case class ConcatAgg(separator: Expression, child: Expression) extends Aggregation {
  ...
  override private[flink] def toAggCall(name: String)(implicit relBuilder: RelBuilder): AggCall =
    relBuilder.aggregateCall(
      getSqlAggFunction, false, false, null,
      name, separator.toRexNode, child.toRexNode)
