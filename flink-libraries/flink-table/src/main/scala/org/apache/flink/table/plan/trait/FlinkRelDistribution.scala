...
object FlinkRelDistribution {
  ...
  : FlinkRelDistribution = {
    ...
    canonize(
      new FlinkRelDistribution(Type.HASH_DISTRIBUTED, list, requireStrict = requireStrict))
  }

  /** Creates a range distribution. */
  def range(collations: JList[RelFieldCollation]): FlinkRelDistribution = {
    val keys = new JArrayList[Integer]()
    for (field: RelFieldCollation <- collations) {
  ...
  def range(columns: util.Collection[_ <: Number]): FlinkRelDistribution = {
    ...
    val collations = new JArrayList[RelFieldCollation]()
    columns.foreach(f => collations.add(RelFieldCollations.of(f.intValue())))
    val fieldCollations = ImmutableList.copyOf[RelFieldCollation](collations)
    canonize(new FlinkRelDistribution(
