...
object ValueInterval {
  ...
  def apply(
    ...
    (lower, upper) match {
      case (lowerV: Comparable[_], upperV: Comparable[_]) =>
        if(lower.getClass != upper.getClass) {
          throw new IllegalArgumentException(
            s"lower: ${lower.getClass} must be same calss as upper: ${upper.getClass}")
        }
      ...
      case (_, null) =>
        throw new IllegalArgumentException(
          s"Class of lower ${lower.getClass} is not subClass of comparable!")
      case (null, upperV: Comparable[_]) =>
        LeftSemiInfiniteValueInterval(upperV, includeUpper)
      case (null, _) =>
        throw new IllegalArgumentException(
          s"Class of upper ${lower.getClass} is not subClass of comparable!")
      case (_, _) =>
        throw new IllegalArgumentException(
  ...
  def contains(interval: ValueInterval, value: Any): Boolean = {
    value match {
      ...
      case _ =>
        throw new IllegalArgumentException(
          s"Class of value ${value.getClass} is not subClass of comparable!")
    }
  }
