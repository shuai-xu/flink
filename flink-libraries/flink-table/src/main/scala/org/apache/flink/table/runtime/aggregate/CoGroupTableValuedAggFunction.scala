...
/**
  * Class of deterministic co-table-valued Aggregate Function
  * used for the groupby (without window) aggregate
  */
class CoGroupTableValuedAggFunction(
  ...
  // BufferedCollector
  private var appendCollector: AppendGroupKeyCollector = _

  override def open(ctx: ExecutionContext): Unit = {
    super.open(ctx)
    appendCollector = new AppendGroupKeyCollector
    LOG.debug(s"Compiling CoGroupTableValuedAggFunction: ${genAggsHandler.name} \n\n " +
                s"Code:\n${genAggsHandler.code}")
  }
