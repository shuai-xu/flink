...
/**
  * This rule is used to coerce the both sides of EQUALS(`=`) operator in Join condition
...
class JoinConditionTypeCoerceRule extends RelOptRule(
  ...
  override def matches(call: RelOptRuleCall): Boolean = {
    ...
    val dataTypeFactory = call.builder().getTypeFactory
    hasEqualsRefsOfDifferentTypes(
      dataTypeFactory,
      join.getCondition)
  }
  ...
  override def onMatch(call: RelOptRuleCall): Unit = {
    ...
    val dataTypeFactory = builder.getTypeFactory
      ...
      case c: RexCall if c.isA(SqlKind.EQUALS) =>
        (c.operands.head, c.operands.last) match {
          case (ref1: RexInputRef, ref2: RexInputRef)
            ...
            val targetType = rexBuilder.getTypeFactory.leastRestrictive(
              refList.map(ref => ref.getType))
  ...
  private def hasEqualsRefsOfDifferentTypes(
    ...
    node: RexNode): Boolean = {
    val joinFilters = RelOptUtil.conjunctions(node)
    joinFilters.exists {
      case c: RexCall if c.isA(SqlKind.EQUALS) =>
        (c.operands.head, c.operands.last) match {
          case (ref1: RexInputRef, ref2: RexInputRef) =>
