...
/**
  * Base class of co-table-valued Aggregate Function used for the groupby (without window) aggregate
  */
abstract class CoGroupTableValuedAggFunctionBase(
    genAggsHandler: GeneratedCoTableValuedAggHandleFunction,
    accTypes: DataType,
    queryConfig: StreamQueryConfig)
  extends CoProcessFunctionWithCleanupState[BaseRow, BaseRow, BaseRow](queryConfig)
    with TableValuedAggFunctionBase
    with Logging {

  protected var function: CoTableValuedAggHandleFunction = _
  ...
  override def open(ctx: ExecutionContext): Unit = {
    ...
    initCleanupTimeState("CoGroupAggregateCleanupTime")
  }
