...
import org.apache.flink.table.api.types.{DataTypes, DecimalType, GenericType, InternalType}
...
object SqlGenUtil {
  ...
  def getSqlText(tableEnv: TableEnvironment): String = {
    if (tableEnv.getConfig.getSubsectionOptimization) {
      getSqlText(tableEnv.sinkNodes, new SqlGenVisitor(tableEnv))
    } else {
      ""
    }
  }
