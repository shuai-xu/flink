...
class SqlGenTest extends TableTestBase {
  ...
  def testUDTFAS(): Unit = {
    ...
    val sql = SqlGenUtil.getSqlText(tEnv)
    println(sql)
  }
  ...
  def testProctimeTumblingWindowAgg(): Unit = {
    ...
    val tableSource = new SimpleTimeAttrTableSource(
      Array("a", "b", "c", "proctime"),
      Array(DataTypes.INT, DataTypes.STRING, DataTypes.INT, DataTypes.TIMESTAMP),
      "proctime"
    )
...
class SimpleTimeAttrTableSource(
  ...
  override def getReturnType: DataType = {
    DataTypes.createRowType(fieldTypes, fieldNames)
  }
