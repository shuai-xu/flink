...
import org.apache.flink.table.plan.optimize.program.{BatchOptimizeContext, FlinkChainedPrograms, FlinkHepRuleSetProgramBuilder, HEP_RULES_EXECUTION_TYPE}
...
class FilterSimplifyExpressionsRuleTest extends TableTestBase {
  ...
  def testSimpleCondition(): Unit = {
    val sqlQuery = "SELECT * FROM x WHERE (a = 1 AND b = 2) OR (NOT(a <> 1) AND c = 3) AND true"
