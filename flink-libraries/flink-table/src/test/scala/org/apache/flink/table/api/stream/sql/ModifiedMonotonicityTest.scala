...
class ModifiedMonotonicityTest extends TableTestBase {
  ...
  def testTumbleFunAndRegularAggFunInGroupBy(): Unit = {
    ...
    val sql =
      "SELECT b, d, weightedAvg(c, a) FROM " +
    ...
    verifyMono(sql, new RelModifiedMonotonicity(Array(CONSTANT, CONSTANT, NOT_MONOTONIC)))
  }
  ...
  def testSemiJoin(): Unit = {
    ...
    verifyMono(sql, new RelModifiedMonotonicity(Array(CONSTANT, INCREASING)))
  }
  ...
  def testUnionAll(): Unit = {
    ...
    verifyMono(sql, new RelModifiedMonotonicity(Array(NOT_MONOTONIC, NOT_MONOTONIC)))
  }
  ...
  def testOver(): Unit = {
    val sql = "SELECT a, " +
      ...
      "    PARTITION BY a ORDER BY proctime ROWS BETWEEN 4 PRECEDING AND CURRENT ROW) " +
      "FROM MyTable"
    verifyMono(sql, new RelModifiedMonotonicity(Array(CONSTANT, CONSTANT, CONSTANT)))
  }

  @Test
  def testMultiOperandsForCalc(): Unit = {
    streamUtil.addFunction("func1", new Func1)
    val sql = "SELECT func1(func1(a1, a3)) from " +
      "(SELECT last_value(a1) as a1, last_value(a3) as a3 FROM AA group by a2) "
    verifyMono(sql, Array(),
      new RelModifiedMonotonicity(Array(NOT_MONOTONIC)))
  }
  ...
  def verifyMono(sql: String, expect: RelModifiedMonotonicity): Unit = {
