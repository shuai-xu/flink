...
abstract class IntegralAvgAggFunction[T] extends AggregateFunction[T, IntegralAvgAccumulator] {
  ...
  override def getAccumulatorType: DataType = {
    DataTypes.createTupleType(classOf[IntegralAvgAccumulator], DataTypes.LONG, DataTypes.LONG)
  }
...
abstract class FloatingAvgAggFunction[T] extends AggregateFunction[T, FloatingAvgAccumulator] {
  ...
  override def getAccumulatorType: DataType = {
    DataTypes.createTupleType(classOf[FloatingAvgAccumulator], DataTypes.DOUBLE, DataTypes.LONG)
  }
...
class DecimalAvgAggFunction(argType: DecimalType)
  ...
  override def getResultType: DecimalType =
