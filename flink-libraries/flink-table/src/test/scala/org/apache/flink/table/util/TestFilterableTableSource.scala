...
class TestFilterableTableSource(val recordNum: Int = 33)
  ...
  val physicalTypes: Array[TypeInformation[_]] = Array(
    STRING,
    LONG,
    INT,
    DOUBLE)

  private val physicalRowType = new RowTypeInfo(physicalTypes, fieldNames)
  ...
  override def getBoundedStream(streamEnv: StreamExecutionEnvironment): DataStream[Row] =
    streamEnv.createInput(
      new CollectionInputFormat[Row](
        generateDynamicCollection().asJava,
        physicalRowType.createSerializer(new ExecutionConfig)),
      physicalRowType, getCallLocationName())
