...
public class ParallelismCalculatorOnStatisticsTest {
	...
	public void setUp() {
		...
		tableConfig.getParameters().setInteger(BatchExecResourceUtil.SQL_EXEC_INFER_RESOURCE_OPERATOR_MIN_PARALLELISM, 5);
		tableConfig.getParameters().setString(TableConfig.SQL_EXEC_INFER_RESOURCE_MODE(), BatchExecResourceUtil.InferMode.ALL.toString());
		...
		when(mq.getAverageRowSize(scanParallelism42)).thenReturn(1.4d * BatchExecResourceUtil.SIZE_IN_MB);
		when(mq.getRowCount(scanParallelismMax)).thenReturn(30000d);
		when(mq.getAverageRowSize(scanParallelismMax)).thenReturn(1.4d * BatchExecResourceUtil.SIZE_IN_MB);
	}
