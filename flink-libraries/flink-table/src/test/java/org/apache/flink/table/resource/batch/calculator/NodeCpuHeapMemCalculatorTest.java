...
/**
 * Test for NodeCpuHeapMemCalculator.
 */
public class NodeCpuHeapMemCalculatorTest extends MockNodeTestBase {
	...
	public void testCalc() {
		...
		connect(5, 2);
		connect(6, 4, 5);
		NodeCpuHeapMemCalculator.calculate(tEnv, nodeList.get(6));
		assertEquals(buildResource(0, 0), nodeList.get(0).getResource());
		assertEquals(buildResource(0, 0), nodeList.get(1).getResource());
		verify((BatchExecTableSourceScan) nodeList.get(1)).setResForSourceAndConversion(buildResourceSpec(0.7, 50), buildResourceSpec(0.5, 20));
		assertEquals(buildResource(0.5d, 20), nodeList.get(3).getResource());
		assertEquals(buildResource(0.5d, 20), nodeList.get(6).getResource());
		assertEquals(buildResource(0, 0), nodeList.get(7).getResource());
		verify((BatchExecTableSourceScan) nodeList.get(7)).setResForSourceAndConversion(buildResourceSpec(0.5, 40), buildResourceSpec(0.5, 20));
	}

	private NodeResource buildResource(double cpu, int heap) {
