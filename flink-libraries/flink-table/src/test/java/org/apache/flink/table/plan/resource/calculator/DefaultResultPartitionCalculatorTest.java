...
/**
 * Test for DefaultResultPartitionCalculator.
 */
public class DefaultResultPartitionCalculatorTest extends MockRelTestBase {
	...
	public void testUnion() {
		 ...
		 *           2, Union
		 */
		tEnv.getConfig().getParameters().setString(TableConfig.SQL_EXEC_INFER_RESOURCE_MODE(), BatchExecResourceUtil.InferMode.NONE.toString());
		...
		connect(2, 0, 1);
		DefaultResultPartitionCalculator.calculate(tEnv, relList.get(2));
	...
	public void testSourceInferModeIsNone() {
		...
		connect(1, 0);
		DefaultResultPartitionCalculator.calculate(tEnv, relList.get(1));
	...
	public void testSourceInferMinParallelism() {
		 ...
		 *    1, Exchange
		 */
		tEnv.getConfig().getParameters().setString(TableConfig.SQL_EXEC_INFER_RESOURCE_MODE(), BatchExecResourceUtil.InferMode.ONLY_SOURCE.toString());
		tEnv.getConfig().getParameters().setInteger(TableConfig.SQL_EXEC_DEFAULT_PARALLELISM(), 50);
		...
		when(mq.getAverageRowSize(scan)).thenReturn(1.6 * BatchExecResourceUtil.SIZE_IN_MB);
		updateRel(0, scan);
		BatchExecExchange execExchange = mock(BatchExecExchange.class, RETURNS_DEEP_STUBS);
		updateRel(1, execExchange);
		when(execExchange.getDistribution().getType()).thenReturn(RelDistribution.Type.BROADCAST_DISTRIBUTED);
	...
	public void testSourceInferMaxParallelism() {
		...
		when(mq.getAverageRowSize(scan)).thenReturn(1d * BatchExecResourceUtil.SIZE_IN_MB);
		updateRel(0, scan);
		BatchExecExchange execExchange = mock(BatchExecExchange.class, RETURNS_DEEP_STUBS);
		updateRel(1, execExchange);
		when(execExchange.getDistribution().getType()).thenReturn(RelDistribution.Type.BROADCAST_DISTRIBUTED);
	...
	public void testSourceInferParallelism() {
		...
		when(mq.getAverageRowSize(scan)).thenReturn(12d * BatchExecResourceUtil.SIZE_IN_MB);
		updateRel(0, scan);
		BatchExecExchange execExchange = mock(BatchExecExchange.class, RETURNS_DEEP_STUBS);
		updateRel(1, execExchange);
		when(execExchange.getDistribution().getType()).thenReturn(RelDistribution.Type.BROADCAST_DISTRIBUTED);
