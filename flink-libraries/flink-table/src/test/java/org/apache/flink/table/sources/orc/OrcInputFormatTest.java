 ...
 * and {@link org.apache.flink.table.sources.orc.RowOrcInputFormat}.
 */
public class OrcInputFormatTest {
	...
	public void testTypesWithoutNulls() throws IOException {
		...
		RowOrcInputFormat inputFormat = new RowOrcInputFormat(new org.apache.flink.core.fs.Path(path), fieldTypes, fieldNames);

		OrcTestUtil.checkOrcRowBaseFormat(fieldTypes, fieldNames, fieldTypes, fieldNames,
			path, split, rowIterator, inputFormat, expertRows);
	}

	@Test
	public void testFetchMissingColumnWithoutNulls() throws IOException {
		InternalType[] fieldTypes = new InternalType[]{
			BooleanType.INSTANCE,
			ByteType.INSTANCE,
			ShortType.INSTANCE,
		...
		InternalType[] requireFieldTypes = new InternalType[]{
			BooleanType.INSTANCE, FloatType.INSTANCE, FloatType.INSTANCE};
		final String[] requireFieldNames = new String[]{"f1", "f22", "f6"};
		...
		Iterator<GenericRow> rowIterator = new OrcTestUtil.GeneratorRow(generatorSize) {
			...
			public GenericRow next() {
				...
				GenericRow request = new GenericRow(requireFieldNames.length);
				request.update(0, boolVal);
				request.update(1, null);
				request.update(2, floatVal);
				expertRows.add(request);
				return r;
			}
		...
		RowOrcInputFormat inputFormat =
			new RowOrcInputFormat(new org.apache.flink.core.fs.Path(path), requireFieldTypes, requireFieldNames);

		OrcTestUtil.checkOrcRowBaseFormat(
			fieldTypes, fieldNames, requireFieldTypes, requireFieldNames,
			path, split, rowIterator, inputFormat, expertRows);
	}
	...
	public void testTypesWithNulls() throws IOException {
		...
		Iterator<GenericRow> rowIterator = new OrcTestUtil.GeneratorRow(generatorSize) {
			...
			public GenericRow next() {
				...
				r.update(7, (index % 1000 == 0) ? null : random.nextInt() + "testTypesWithNulls");
				expertRows.add(r);
