...
import org.apache.flink.table.runtime.operator.window.assigners.TumblingWindowAssigner;
...
import static org.apache.flink.table.runtime.operator.window.WindowTestUtils.record;
import static org.apache.flink.table.runtime.operator.window.WindowTestUtils.retractRecord;
...
public class WindowOperatorTest {
	...
	private InternalType[] windowTypes = new InternalType[]{DataTypes.LONG, DataTypes.LONG, DataTypes.LONG};
	private GenericRowEqualiser equaliser = new GenericRowEqualiser(accTypes, windowTypes);
	private BinaryRowKeySelector keySelector = new BinaryRowKeySelector(new int[]{0}, inputType);
	...
	public void testEventTimeSlidingWindows() throws Exception {
		...
		OneInputStreamOperatorTestHarness<BaseRow, BaseRow> testHarness = new KeyedOneInputStreamOperatorTestHarness<BaseRow, BaseRow, BaseRow>(
	...
	public void testProcessingTimeSessionWindows() throws Throwable {
		...
		BaseRowHarnessAssertor assertor = new BaseRowHarnessAssertor(outputType.getFieldTypes(), new GenericRowResultSortComparator());
	...
	public void testTumblingCountWindow() throws Exception {
		...
		InternalType[] windowTypes = new InternalType[]{DataTypes.LONG};
	...
	private static class GenericRowResultSortComparator implements Comparator<GenericRow>, Serializable {
		...
		public int compare(GenericRow row1, GenericRow row2) {
			String key1 = row1.getString(0);
			String key2 = row2.getString(0);
	...
	private static class SumAndCountAggTimeWindow extends SumAndCountAgg<TimeWindow> {
		...
		public BaseRow getValue(TimeWindow namespace) throws Exception {
			...
			row.update(2, namespace.getStart());
			row.update(3, namespace.getEnd());
			row.update(4, namespace.maxTimestamp());
			return row;
		}
	}
	...
	private static class SumAndCountAggCountWindow extends SumAndCountAgg<CountWindow> {
		...
		public BaseRow getValue(CountWindow namespace) throws Exception {
			...
			row.update(2, namespace.getId());
			return row;
		}
	}

	private static class SumAndCountAgg<W extends Window> implements SubKeyedAggsHandleFunction<W> {
