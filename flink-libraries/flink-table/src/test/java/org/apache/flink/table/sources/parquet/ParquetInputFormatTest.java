...
/**
 * Tests for {@link org.apache.flink.table.sources.parquet.ParquetInputFormat}
 * and {@link RowParquetInputFormat}.
 */
public class ParquetInputFormatTest {
	...
	public void testIllegalConstructorArguments() {
		...
		try {
			// empty field name
			new RowParquetInputFormat(path, new InternalType[]{DataTypes.INT}, new String[0]);
			assertFalse(true);
		...
		try {
			// empty field type
			new RowParquetInputFormat(path, new InternalType[0], new String[]{"f1", "f2"});
			assertFalse(true);
		} catch (IllegalArgumentException e) {
			assertTrue(true);
		}
		...
		try {
			...
			new RowParquetInputFormat(
					path,
					new InternalType[]{DataTypes.INT},
					new String[]{"f1", "f2"});
			assertFalse(true);
		} catch (IllegalArgumentException e) {
			assertTrue(true);
	...
	public void testSplitOneFileWithOneRowGroup() throws IOException {
		...
		RowParquetInputFormat inputFormat = new RowParquetInputFormat(
				...
				new InternalType[]{DataTypes.DOUBLE},
				new String[]{"f1"});
	...
	public void testSupportedTypeInfos() throws IOException {
		...
		assertEquals(TimeConvertUtils.internalToDate(1234567), row.getField(7));
	...
	public void testFieldNameNotFound() throws IOException {
		...
		RowParquetInputFormat inputFormat = new RowParquetInputFormat(
				...
				new InternalType[]{DataTypes.INT, DataTypes.DOUBLE},
				new String[]{"f1", "f4"});
		FileInputSplit[] splits = inputFormat.createInputSplits(1);
		assertEquals(1, splits.length);
		inputFormat.open(splits[0]);
	...
	public void testSchemaNotMatch() throws IOException {
		...
		RowParquetInputFormat inputFormat = new RowParquetInputFormat(
				new org.apache.flink.core.fs.Path(file),
				new InternalType[]{DataTypes.DOUBLE, DataTypes.STRING},
				new String[]{"f1", "f2"});
		FileInputSplit[] splits = inputFormat.createInputSplits(1);
		assertEquals(1, splits.length);
		inputFormat.open(splits[0]);
