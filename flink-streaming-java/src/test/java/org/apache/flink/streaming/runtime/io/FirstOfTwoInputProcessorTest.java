...
public class FirstOfTwoInputProcessorTest {
	...
	public void testHandleWatermark() throws Exception {
		...
		assertEquals(Long.MIN_VALUE, processor.getWatermarkProcessor().getInput1WatermarkGauge().getValue().longValue());

		final Watermark streamElement2 = new Watermark(234L);
		processor.processWatermark(streamElement2, 0);
		assertEquals(123L, processor.getWatermarkProcessor().getInput1WatermarkGauge().getValue().longValue());
