...
/**
 * Test for {@link FirstOfTwoInputProcessor}.
 */
public class FirstOfTwoInputProcessorTest {
	...
	public void testHandleWatermark() throws Exception {
		...
		final FirstOfTwoInputProcessor processor = new FirstOfTwoInputProcessor(
			subMaintainer,
			operator,
			this,
			taskMetricGroup,
		...
		assertEquals(Long.MIN_VALUE, processor.getInput1WatermarkGauge().getValue().longValue());

		final StreamElement streamElement2 = new Watermark(234L);
		processor.processElement(streamElement2, 0);
		assertEquals(123L, processor.getInput1WatermarkGauge().getValue().longValue());

		verify(metricGroup, times(1)).gauge(MetricNames.IO_CURRENT_INPUT_1_WATERMARK, processor.getInput1WatermarkGauge());
		verify(operator, times(1)).processWatermark1(streamElement1.asWatermark());
		assertEquals(0, taskCounter.getCount());
	}
	...
	public void testProcessLatencyMarker() throws Exception {
		...
		verify(operator, times(2)).processLatencyMarker1(any(LatencyMarker.class));
		assertEquals(0, taskCounter.getCount());
	}
