...
public class HBaseTypeUtils {
	...
	public static Object deserializeToInnerObject(byte[] value, int typeIdx, Charset stringCharset) {
		switch (typeIdx) {
			case 0: // byte[]
				return value;
			case 1: //BinaryString
			...
			case 9: // sql.Timestamp encoded as long
				return InternalTypeConverters.TimestampConverter$.MODULE$.toInternal(new Timestamp(Bytes.toLong(value)));
			case 10: // sql.Date encoded as long
				return InternalTypeConverters.DateConverter$.MODULE$.toInternal(new Date(Bytes.toLong(value)));
			case 11: // sql.Time encoded as long
				return InternalTypeConverters.TimeConverter$.MODULE$.toInternal(new Time(Bytes.toLong(value)));
			case 12:
	...
	public static byte[] serializeFromInternalObject(Object value, int typeIdx, Charset stringCharset) {
		switch (typeIdx) {
			case 0: // byte[]
				return (byte[]) value;
			case 1: //binaryString
			...
			case 9: // sql.Timestamp encoded as long
				return Bytes.toBytes((InternalTypeConverters.TimestampConverter$.MODULE$.toExternalImpl(value)).getTime());
			case 10: // sql.Date encoded as long
				return Bytes.toBytes((InternalTypeConverters.DateConverter$.MODULE$.toExternalImpl(value)).getTime());
			case 11: // sql.Time encoded as long
				return Bytes.toBytes((InternalTypeConverters.TimeConverter$.MODULE$.toExternalImpl(value)).getTime());
	...
	// BinaryString
	public static byte[] toBytes(BinaryString string, Charset stringCharset) {
		return string.toString().getBytes(stringCharset);
	}

	public static byte[] toBytes(BinaryString string) {
		return string.toString().getBytes(DEFAULT_CHARSET);
	}

	public static BinaryString fromBytes(byte[] bytes, Charset stringCharset) {
