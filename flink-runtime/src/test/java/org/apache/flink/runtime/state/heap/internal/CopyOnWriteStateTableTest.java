...
public class CopyOnWriteStateTableTest extends TestLogger {
	...
	public void testPutGetRemoveContainsTransform() throws Exception {
		...
		Assert.assertNull(stateTable.putAndGetOld(1, 1, state11));
		Assert.assertEquals(state11, stateTable.get(1, 1));
		Assert.assertEquals(1, stateTable.size());

		Assert.assertNull(stateTable.putAndGetOld(2, 1, state21));
		Assert.assertEquals(state21, stateTable.get(2, 1));
		Assert.assertEquals(2, stateTable.size());

		Assert.assertNull(stateTable.putAndGetOld(1, 2, state12));
		Assert.assertEquals(state12, stateTable.get(1, 2));
		...
		stateTable.put(2, 1, state21);
		Assert.assertEquals(3, stateTable.size());

		Assert.assertEquals(state21, stateTable.removeAndGetOld(2, 1));
