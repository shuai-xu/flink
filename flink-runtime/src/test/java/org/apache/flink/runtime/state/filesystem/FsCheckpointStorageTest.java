...
public class FsCheckpointStorageTest extends AbstractFileCheckpointStorageTestBase {
	...
	protected CheckpointStorage createCheckpointStorage(Path checkpointDir) throws Exception {
		return new FsCheckpointStorage(checkpointDir, null, new JobID(), createCheckpointSubDir, FILE_SIZE_THRESHOLD, WRITE_BUFFER_SIZE);
	}
	...
	protected CheckpointStorage createCheckpointStorageWithSavepointDir(Path checkpointDir, Path savepointDir) throws Exception {
		return new FsCheckpointStorage(checkpointDir, savepointDir, new JobID(), createCheckpointSubDir, FILE_SIZE_THRESHOLD, WRITE_BUFFER_SIZE);
	}
	...
	public void testSavepointsInOneDirectoryDefaultLocation() throws Exception {
		...
		final FsCheckpointStorage storage = new FsCheckpointStorage(
				Path.fromLocalFile(tmp.newFolder()), defaultSavepointDir, new JobID(), createCheckpointSubDir, FILE_SIZE_THRESHOLD, WRITE_BUFFER_SIZE);
	...
	public void testSavepointsInOneDirectoryCustomLocation() throws Exception {
		...
		final FsCheckpointStorage storage = new FsCheckpointStorage(
				Path.fromLocalFile(tmp.newFolder()), null, new JobID(), createCheckpointSubDir, FILE_SIZE_THRESHOLD, WRITE_BUFFER_SIZE);
	...
	public void testTaskOwnedStateStream() throws Exception {
		...
		final FsCheckpointStorage storage = new FsCheckpointStorage(
				Path.fromLocalFile(tmp.newFolder()),  null, new JobID(), createCheckpointSubDir, 10, WRITE_BUFFER_SIZE);
	...
	public void testStorageLocationDoesNotMkdirs() throws Exception {
		FsCheckpointStorage storage = new FsCheckpointStorage(
				randomTempPath(), null, new JobID(), createCheckpointSubDir, FILE_SIZE_THRESHOLD, WRITE_BUFFER_SIZE);
