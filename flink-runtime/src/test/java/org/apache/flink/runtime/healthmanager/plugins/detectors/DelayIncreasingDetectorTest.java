...
/**
 * Tests to check whether delay is increasing or not.
 */
public class DelayIncreasingDetectorTest extends DetectorTestBase  {

	@Test
	public void testDetectNotIncreasingDelay() throws Exception {
			...
			public TaskMetricSubscription answer(InvocationOnMock invocation) throws Throwable {
				...
				if (vertex1.equals(vertexId)) {
					...
					if (metricName.equals(MetricNames.SOURCE_DELAY)) {
						if (timelineAggType.equals(TimelineAggType.DELTA_MIN)) {
		...
		DelayIncreasingDetector detector = new DelayIncreasingDetector();
		detector.open(monitor);
		assertNull(detector.detect());
	}

	@Test
	public void testDetectIncreasingDelay() throws Exception {
		// initial job vertex config.
		JobVertexID vertex1 = new JobVertexID();
		JobVertexID vertex2 = new JobVertexID();
		Map<JobVertexID, RestServerClient.VertexConfig> vertexConfigs = new HashMap<>();
		...
		DelayIncreasingDetector detector = new DelayIncreasingDetector();
		detector.open(monitor);
		JobVertexDelayIncreasing delayIncreasing = (JobVertexDelayIncreasing) detector.detect();
		List<JobVertexID> vertex = new LinkedList<>();
		vertex.add(vertex1);
		assertEquals(vertex, delayIncreasing.getJobVertexIDs());
	}
}
