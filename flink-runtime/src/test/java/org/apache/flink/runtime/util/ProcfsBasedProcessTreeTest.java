...
import static org.apache.flink.runtime.util.ProcfsBasedProcessTree.KB_TO_BYTES;
import static org.apache.flink.runtime.util.ProcfsBasedProcessTree.UNAVAILABLE;
...
public class ProcfsBasedProcessTreeTest {
	...
	private static final Logger LOG = LoggerFactory.getLogger(ProcfsBasedProcessTreeTest.class);
	@ClassRule
	...
	private class RogueTaskThread extends Thread {
		public void run() {
			try {
				...
				LOG.info("Error executing shell command ", e);
			} finally {
	...
	public void setup() throws IOException {
		...
		TEST_ROOT_DIR = tempFolder.newFolder();
	}
	...
	public void testProcessTree() throws Exception {
		...
		FileUtils.writeFileUtf8(file, "# rogue task\n" + "sleep 1\n" + "echo hello\n"
				+ "if [ $1 -ne 0 ]\n" + "then\n" + " sh " + shellScript
	...
	protected ProcfsBasedProcessTree createProcessTree(String pid,
		...
		return new ProcfsBasedProcessTree(pid, procfsRootDir, clock, false);
	}

	protected void destroyProcessTree(String pid) throws IOException, InterruptedException {
		sendSignal("-" + pid, 9);
	}
	...
	public ProcfsBasedProcessTree.ProcessSmapMemoryInfo constructMemoryMappingInfo(String address,
																				   String[] entries) {
	...
	public void createMemoryMappingInfo(ProcfsBasedProcessTree.ProcessTreeSmapMemInfo[] procMemInfo) {
		for (int i = 0; i < procMemInfo.length; i++) {
			...
			List<ProcfsBasedProcessTree.ProcessSmapMemoryInfo> memoryMappingList =
	...
	public void testCpuAndMemoryForProcessTree() throws IOException {
		...
		try {
			...
			ProcfsBasedProcessTree.ProcessTreeSmapMemInfo[] memInfo = new ProcfsBasedProcessTree.ProcessTreeSmapMemInfo[4];
			...
			testClock.setNanoTime(elapsedTimeBetweenUpdatesMsec * 1000);
			// build the process tree.
			processTree.updateProcessTree();
			...
			FileUtils.deleteDirectoryQuietly(procfsRootDir);
		}
	}

	private void setSmapsInProceTree(ProcfsBasedProcessTree processTree,
									 boolean enableFlag) {
		processTree.setSmapsEnabled(enableFlag);
		processTree.updateProcessTree();
	}
		...
		try {
			...
			newProcInfos[0] =
				new ProcessStatInfo(new String[]{"500", "proc5", "100", "100",
					"100", "500000", "500"});
			ProcfsBasedProcessTree.ProcessTreeSmapMemInfo[] newMemInfos = new ProcfsBasedProcessTree.ProcessTreeSmapMemInfo[1];
			...
			if (!smapEnabled) {
				long cumuRssMem =
					ProcfsBasedProcessTree.PAGE_SIZE > 0
						? 1200L * ProcfsBasedProcessTree.PAGE_SIZE : UNAVAILABLE;
			...
			if (!smapEnabled) {
				long cumuRssMem =
					ProcfsBasedProcessTree.PAGE_SIZE > 0
						? 700L * ProcfsBasedProcessTree.PAGE_SIZE : UNAVAILABLE;
				Assert.assertEquals(
	...
	public void testProcessTreeDump() throws IOException {
		...
		try {
			...
			ProcfsBasedProcessTree.ProcessTreeSmapMemInfo[] memInfos = new ProcfsBasedProcessTree.ProcessTreeSmapMemInfo[6];
	...
	protected static boolean isSetsidAvailable() {
		...
		try {
			...
			LOG.warn("setsid is not available on this machine. So not using it.", e);
	...
	private static boolean isAlive(String pid) throws IOException, InterruptedException {
		...
		final int exitCode = sendSignal(sigpid, 0);
		return exitCode == 0;
	}

	private static int sendSignal(String pid, int signal) throws IOException, InterruptedException {
		ShellCommandExecutor shexec = null;
	...
	private static boolean isAnyProcessInTreeAlive(
		ProcfsBasedProcessTree processTree) throws IOException, InterruptedException {
	...
	public static void setupProcfsRootDir(File procfsRootDir) throws IOException {
		...
		if (procfsRootDir.exists()) {
			FileUtils.deleteFileOrDirectory(procfsRootDir);
		}
	...
	public static void setupPidDirs(File procfsRootDir, String[] pids)
		...
		for (String pid : pids) {
			...
			FileUtils.deleteFileOrDirectory(pidDir);
	...
	public static void writeStatFiles(File procfsRootDir, String[] pids,
									  ProcessStatInfo[] procs, ProcfsBasedProcessTree.ProcessTreeSmapMemInfo[] smaps)
		throws IOException {
