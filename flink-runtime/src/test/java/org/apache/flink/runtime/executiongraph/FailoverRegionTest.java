...
public class FailoverRegionTest extends TestLogger {
	...
	public void testMultiRegionsFailover() throws Exception {
		...
		ev21.scheduleForExecution(scheduler, true);
		...
		ev22.scheduleForExecution(scheduler, true);
		ev22.getCurrentExecutionAttempt().markFinished();
		assertEquals(JobStatus.RUNNING, strategy.getFailoverRegion(ev11).getState());
		assertEquals(JobStatus.RUNNING, strategy.getFailoverRegion(ev22).getState());
		assertEquals(JobStatus.RUNNING, strategy.getFailoverRegion(ev3).getState());

		ev3.getCurrentExecutionAttempt().fail(new Exception("New fail"));
		assertEquals(JobStatus.RUNNING, strategy.getFailoverRegion(ev11).getState());
		assertEquals(JobStatus.RUNNING, strategy.getFailoverRegion(ev22).getState());
		assertEquals(JobStatus.CANCELLING, strategy.getFailoverRegion(ev3).getState());

		for (ExecutionVertex ev : eg.getJobVertex(v3.getID()).getTaskVertices()) {
