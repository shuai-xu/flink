...
public class WebMonitorEndpoint<T extends RestfulGateway> extends RestServerEndpoint implements LeaderContender, JsonArchivist {
	...
	protected List<Tuple2<RestHandlerSpecification, ChannelInboundHandler>> initializeHandlers(CompletableFuture<String> restAddressFuture) {
		...
		final UpdatingJobHandlers updatingJobHandlers = new UpdatingJobHandlers();

		final UpdatingJobHandlers.UpdatingTriggerHandler updatingTriggerHandler = updatingJobHandlers.new UpdatingTriggerHandler(
			restAddressFuture,
			leaderRetriever,
			timeout,
			responseHeaders
		...
		final UpdatingJobHandlers.UpdatingStatusHandler updatingStatusHandler = updatingJobHandlers.new UpdatingStatusHandler(
			restAddressFuture,
			leaderRetriever,
			timeout,
			responseHeaders
