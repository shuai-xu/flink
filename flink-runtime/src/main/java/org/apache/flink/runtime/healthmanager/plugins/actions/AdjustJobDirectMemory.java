...
/**
 * Adjust direct memory for given vertex.
 */
public class AdjustJobDirectMemory extends AdjustJobResource {
	public AdjustJobDirectMemory(JobID jobID, long timeoutMs) {
			...
			if (mergedAction.targetResource.containsKey(vertexId)) {
				mergedAction.targetResource.put(vertexId,
					new ResourceSpec.Builder(mergedAction.targetResource.get(vertexId))
						.setDirectMemoryInMB(this.targetResource.get(vertexId).getDirectMemory()).build());
			} else {
