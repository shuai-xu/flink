...
object UpsertPvUvSQL {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val outPath = if (params.has("output"))
      params.get("output")
    else {
      System.out.println("Use --output to specify output path. Printing result to tmp dir.")
      System.getProperty("java.io.tmpdir") + "/pvuv"
    }
