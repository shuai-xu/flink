...
/**
 * A sorted iterator over the values in Niagara internal state.
 ...
 */
public abstract class AbstractNiagaraSortedPartitionIterator<T> implements Iterator<T> {
	...
	private final Collection<Iterator<NiagaraEntry>> groupIterators;

	/** The internal state descriptor of this state. */
	private final InternalStateDescriptor descriptor;

	public AbstractNiagaraSortedPartitionIterator(
		Collection<Iterator<NiagaraEntry>> groupIterators,
	...
	private void initGroupNodes() {
		...
		for (Iterator<NiagaraEntry> groupIterator : groupIterators) {
	...
	class GroupIterateNode implements Comparable<GroupIterateNode> {
		...
		/** The iterator over the niagara entry in the group. */
		private final Iterator<NiagaraEntry> iterator;

		/** The current niagara entry pointed by the iterator. */
		private volatile NiagaraEntry currentEntry;

		private GroupIterateNode(Iterator<NiagaraEntry> iterator) {
