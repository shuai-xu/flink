...
import static com.alibaba.blink.state.niagara.NiagaraInternalState.GROUP_BYTES_TO_SKIP;

/**
 * The Niagara entry of (dbKey, dbValue).
 */
public class NiagaraEntry {

	private final NiagaraTabletInstance tabletInstance;
	...
	NiagaraEntry(final NiagaraTabletInstance dbInstance, final byte[] dbKey, final byte[] dbValue) {
		...
		this.tabletInstance = dbInstance;
		this.dbKey = dbKey;
	...
	Pair<Row, Row> getRowPair(InternalStateDescriptor descriptor, int prefixKeyLength) {
		...
		return new Pair<Row, Row>() {
			...
			public Row getValue() {
				...
				if (value == null) {
					if (descriptor.getValueMerger() != null) {
						List<Row> rows = NiagaraInternalState.deserializeStateValues(dbValue, descriptor);
						value = NiagaraInternalState.mergeMultiValues(rows, descriptor.getValueMerger());
					} else {
						value = NiagaraInternalState.deserializeStateValue(dbValue, descriptor);
					}
				}
				return value;
			}
			...
			public Row setValue(Row value) {
				...
				byte[] valueBytes = NiagaraInternalState.serializeStateValue(value, descriptor);
				tabletInstance.put(dbKey, valueBytes);
				this.value = value;
				return oldValue;
			}
		};
	...
	private Row deserializeStateKey(byte[] bytes, InternalStateDescriptor descriptor, int prefixKeyLength) {
		try {
			...
			return NiagaraInternalState.deserializeKeyRow(descriptor, inputView);
		} catch (IOException e) {
	...
	private Row deserializeStateValue(byte[] dbValue, InternalStateDescriptor descriptor, RowSerializer dupValueSerializer) {
		...
		try {
			...
			if (rows.size() > 1) {
				...
				return NiagaraInternalState.mergeMultiValues(rows, merger);
			} else {
				return rows.get(0);
			}
		} catch (IOException e) {
