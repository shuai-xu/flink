...
/**
 * Full restore operation for Niagara InternalStateBackend.
 */
public class NiagaraFullRestoreOperation {

	private static final Logger LOG = LoggerFactory.getLogger(NiagaraFullRestoreOperation.class);

	private final NiagaraInternalStateBackend stateBackend;

	NiagaraFullRestoreOperation(NiagaraInternalStateBackend stateBackend) {
	...
	public void restore(
		...
		for (StatePartitionSnapshot rawSnapshot : restoredSnapshots) {
			...
			try {
				inputStream = snapshotHandle.openInputStream();
				stateBackend.getCancelStreamRegistry().registerCloseable(inputStream);
				...
				if (stateBackend.getCancelStreamRegistry().unregisterCloseable(inputStream)) {
					try {
						inputStream.close();
					} catch (Exception e) {
