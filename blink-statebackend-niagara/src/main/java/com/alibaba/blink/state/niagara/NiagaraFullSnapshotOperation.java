...
import static com.alibaba.blink.state.niagara.NiagaraInternalState.KEY_END_BYTE;


/**
 * Full snapshot related operations of Niagara state backend.
 */
public class NiagaraFullSnapshotOperation {

	private static final Logger LOG = LoggerFactory.getLogger(NiagaraFullSnapshotOperation.class);
	...
	NiagaraFullSnapshotOperation(
		NiagaraInternalStateBackend stateBackend,
		long checkpointId,
		...
		this.dbLease = stateBackend.niagaraResourceGuard.acquireResource();
		this.states = new HashMap<>();
		this.duplicatedKVSerializers = new HashMap<>();
	}

	/**
	 * 1) Create a snapshot object from Niagara.
	 *
	 */
	public void takeDBSnapShot() throws IOException, NiagaraException {
		...
		LOG.info("Taking snapshot for Niagara instance at {}.", snapshotDirectory.toString());
	...
	/**
	 * 3) Write the actual data from Niagara from the time we took the snapshot object in (1).
	 *
	 * @throws IOException
	 */
	...
	/**
	 * 5) Release the snapshot object for Niagara and clean up.
	 */
	...
	private void materializeKVStateData() throws NiagaraException, IOException {
		File backupPath = new File(snapshotDirectory.getDirectory().getPath());
		NiagaraTabletInstance backupInstance = new NiagaraTabletInstance(
		...
		try {
			...
			for (int group : groups) {
				...
				for (InternalState state : states.values()) {
					...
					byte[] stateNameBytes = ((NiagaraInternalState) state).stateNameBytes;
					...
					Iterator<Pair<Row, Row>> iterator =
						new NiagaraStateRangeIterator<Pair<Row, Row>>(backupInstance, groupPrefix, groupPrefixEnd) {
							@Override
