...
import static com.alibaba.blink.state.niagara.NiagaraTabletInstance.SST_FILE_SUFFIX;

/**
 * Incremental snapshot related operations of Niagara state backend, this behaves better then {@link NiagaraFullSnapshotOperation} in general.
 */
public class NiagaraIncrementalSnapshotOperation {

	private static final Logger LOG = LoggerFactory.getLogger(NiagaraIncrementalSnapshotOperation.class);
	...
	/** Local directory for the Niagara native backup. */
	...
	NiagaraIncrementalSnapshotOperation(
		...
		this.dbLease = this.stateBackend.niagaraResourceGuard.acquireResource();
	...
	void takeSnapshot() throws Exception {
		...
		LOG.trace("Local Niagara checkpoint goes to backup path {}.", localBackupDirectory);
				...
				if (fileName.endsWith(SST_FILE_SUFFIX)) {
					...
					if (existsAlready) {
						...
						if (streamStateHandle instanceof FileStateHandle) {
							...
							throw new UnsupportedOperationException("Niagara incremental snapshot operation cannot support non FileStateHandle/ByteStreamStateHandle");
						}
	...
	void releaseResources(boolean canceled) {
		...
		try {
			if (localBackupDirectory.exists()) {
				LOG.trace("Running cleanup for local Niagara backup directory {}.", localBackupDirectory);
				boolean cleanupOk = localBackupDirectory.cleanup();

				if (!cleanupOk) {
					LOG.debug("Could not properly cleanup local Niagara backup directory.");
				}
			}
		} catch (IOException e) {
			LOG.warn("Could not properly cleanup local Niagara backup directory.", e);
		}
