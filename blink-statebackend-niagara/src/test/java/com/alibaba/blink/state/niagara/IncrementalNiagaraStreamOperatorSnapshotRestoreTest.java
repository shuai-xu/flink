...
/**
 * Test snapshot/restore of stream operators for Niagara (incremental snapshots).
 * TODO re-enable this UT after keyed state backend is replaced by internal state backend.
 */
@Ignore
public class IncrementalNiagaraStreamOperatorSnapshotRestoreTest extends StreamOperatorSnapshotRestoreTest {

	@Override
	protected StateBackend createStateBackend() throws IOException {
		FsStateBackend stateBackend = createStateBackendInternal();
		return new NiagaraStateBackend(stateBackend, TernaryBoolean.fromBoolean(true), new NiagaraConfiguration());
	}
}
