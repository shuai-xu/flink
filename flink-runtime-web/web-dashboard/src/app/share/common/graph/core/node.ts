...
import { GroupNode, Node, NodeType, ROOT_NAME } from './interface';
import { computeCXPositionOfNodeShape } from './layout';
import { RenderGraphInfo, RenderGroupNodeInfo, RenderNodeInfo } from './render';
import {
  buildGroupScene,
  positionButton,
  positionRect,
...
export function buildGroupNode(sceneGroup, nodeData: RenderNodeInfo[], sceneElement) {
  const container = selectOrCreateChild(sceneGroup, 'g', Class.Node.CONTAINER);
  ...
  .merge(nodeGroups)
  // ENTER + UPDATE
  .attr('class', d => Class.Node.GROUP + ' ' + nodeClass(d))
  .each(function (d) {
    const nodeGroup = d3.select(this);

    // Build .shape first (background of the node).
    const shape = buildShape(nodeGroup, d, Class.Node.SHAPE, sceneElement);
    ...
    subsceneBuild(nodeGroup, <RenderGroupNodeInfo>d, sceneElement);
...
export function nodeClass(d: RenderNodeInfo) {
  switch (d.node.type) {
    case NodeType.OP:
      return Class.OPNODE;
    case NodeType.META:
 ...
 * as the shape's data.
 */
export function buildShape(nodeGroup, d, className: string, sceneElement): d3.Selection<any, any, any, any> {
  // Create a group to house the underlying visual elements.
  const shapeGroup = selectOrCreateChild(nodeGroup, 'g', className);
 ...
 */
function addButton(selection, d: RenderNodeInfo, sceneElement) {
  const group =
          selectOrCreateChild(selection, 'g', Class.Node.BUTTON_CONTAINER);
 ...
 */
function addInteraction(selection, d: RenderNodeInfo,
...
function subsceneBuild(nodeGroup,
                       renderNodeInfo: RenderGroupNodeInfo, sceneElement) {
  if (renderNodeInfo.node.isGroupNode) {
    if (renderNodeInfo.expanded) {
      // Recursively buildDef the subscene.
      return buildGroupScene(nodeGroup, renderNodeInfo, sceneElement,
...
function labelSubtitlePosition(nodeGroup, cx: number, cy: number,
                               yOffset: number) {
 ...
 */
export function stylize(nodeGroup, renderInfo: RenderNodeInfo,
  ...
  const isExtract = renderInfo.isInExtract ||
    renderInfo.isOutExtract;
  const isExpanded = renderInfo.expanded && className !== Class.Annotation.NODE;
  ...
  const node = nodeGroup.select('.' + className + ' .' + Class.Node.COLOR_TARGET);
...
function position(nodeGroup, d: RenderNodeInfo) {
  const shapeGroup = selectChild(nodeGroup, 'g', Class.Node.SHAPE);
  const cx = computeCXPositionOfNodeShape(d);
  switch (d.node.type) {
    case NodeType.OP: {
      const shape = selectChild(shapeGroup, 'foreignObject');
    ...
    case NodeType.META: {
      // position shape
      const shapes = shapeGroup.selectAll('foreignObject');
      if (d.expanded) {
    ...
    case NodeType.SERIES: {
      const shape = selectChild(shapeGroup, 'use');
      if (d.expanded) {
    ...
    case NodeType.BRIDGE: {
      ...
      const shape = selectChild(shapeGroup, 'rect');
      positionRect(shape, d.x, d.y, d.width, d.height);
      break;
    }
    default: {
 ...
 */
function subscenePosition(nodeGroup, d: RenderNodeInfo) {
  const x0 = d.x - d.width / 2.0 + d.paddingLeft;
  const y0 = d.y - d.height / 2.0 + d.paddingTop;

  const subscene = selectChild(nodeGroup, 'g', Class.Subscene.GROUP);
  translate(subscene, x0, y0);
}


export function traceInputs(renderGraphInfo: RenderGraphInfo) {
  ...
  const nodeName = currentNode.getAttribute('data-name');
  const opNodes = _getAllContainedOpNodes(nodeName, renderGraphInfo);
  let allTracedNodes = {};
...
export function traceFirstOutputsOfOpNode(
  renderGraphInfo: RenderGraphInfo, node: BaseNode | GroupNode, edgesSelector: d3.Selection<any , any, any, any>) {
  console.log(node);
  edgesSelector.each(function (d) {
    if (d.v === node.name) {
      ...
      const nextOpNode = renderGraphInfo.getRenderNodeByName(d.w);
...
export function _getAllContainedOpNodes(
  nodeName: string, renderGraphInfo: RenderGraphInfo) {
  let opNodes = [];

  // Get current node.
  const node = renderGraphInfo.getNodeByName(nodeName) as GroupNode | BaseNode;

  if (node.isGroupNode) {
    // Otherwise, make recursive call for each node contained by the GroupNode.
    const childNodeNames = (node as GroupNode).metagraph.nodes();
...
export function traceAllInputsOfOpNode(
  renderGraphInfo: RenderGraphInfo, startNode: BaseNode,
  ...
  // Extract the inputs.
  const inputs = startNode.inputs;
  // Get visible parent.
  const currentVisibleParent = getVisibleParent(renderGraphInfo, startNode);
  // Mark as input node.
  ...
  _.each(inputs, function (nodeInstance) {
    const resolvedNode = renderGraphInfo.getNodeByName(nodeInstance.name);
    ...
    const visibleParent = getVisibleParent(renderGraphInfo, resolvedNode);
  ...
  const startNodeParents = {};
  const indexedStartNodeParents = [ currentVisibleParent ];
  ...
  _.forOwn(visibleInputs, function (visibleParentInfo: VisibleParent, key) {
    const nodeInstance = visibleParentInfo.visibleParent;
    // Make recursive call for each input-OpNode contained by the visible
    // parent.
    _.each(visibleParentInfo.opNodes, function (opNode: BaseNode) {
      allTracedNodes =
...
export function getVisibleParent(
  ...
  while (!found) {
    ...
    if (currentParent === undefined) {
      ...
      const renderNode = renderGraphInfo.getRenderNodeByName(currentParent.name);
...
function _findVisibleParentsFromOpNodes(renderGraphInfo, nodeNames: string[]) {
  const visibleParents: { [ nodeName: string ]: Node } = {};
  _.each(nodeNames, function (nodeName) {
    const currentNode = renderGraphInfo.getNodeByName(nodeName);
    const visibleParent = getVisibleParent(renderGraphInfo, currentNode);
...
function _createVisibleTrace(
  ...
  const startNodeIndex = startNodeParents[ currentNode.name ].index;
  const startNodeName =
          indexedStartNodeParents[ Math.max(startNodeIndex - 1, 0) ].name;

  const startNodeTopParentName = startNodeName;
  const targetNodeTopParentName = previousNode.name;

  const endNodeName = previousNode.name;
  ...
  _.each(destinationParentPairs, function (value) {
    const inner = value[ 0 ];
    const outer = value[ 1 ];
    const edgeSelector = `[data-edge="${inner.name}--${startNodeTopParentName}` +
      `~~${outer.name}~~OUT"]`;
  ...
  for (let index = 1; index < startNodeIndex; index++) {
    const inner = indexedStartNodeParents[ index - 1 ];
    const outer = indexedStartNodeParents[ index ];
    const edgeSelector = `[data-edge="${targetNodeTopParentName}~~${outer.name}` +
      `~~IN--${inner.name}"]`;
    d3.selectAll(edgeSelector).classed('input-edge-highlight', true);
  }
}
